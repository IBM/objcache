...
func NewGCS3(bucket string, debugS3 bool, config *S3Config, bufferSize int) (*S3Backend, error) {
	s3Backend, err := NewS3(bucket, debugS3, config, bufferSize)
	if err != nil {
		return nil, err
	}
	s3Backend.Capabilities().Name = "gcs"
	s3Backend.gcs = true
	s3Backend.cap.NoParallelMultipart = true
...
func (s *GCS3) MultipartBlobAdd(param *MultipartBlobAddInput) (*MultipartBlobAddOutput, error) {
	...
	if commitData.Prev != nil {
		if commitData.Prev.Size == 0 || commitData.Prev.Size%(256*1024) != 0 {
			s3Log.Errorf("putSize of each block must be multiple of 256KB: %v", param.Size)
