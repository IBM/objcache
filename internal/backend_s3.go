...
func NewS3(bucket string, debugS3 bool, config *S3Config, bufferSize int) (*S3Backend, error) {
	awsConfig, err := config.ToAwsConfig(debugS3, bufferSize)
	if err != nil {
		s3Log.Errorf("Failed: NewS3, bucket=%v, err=%v", bucket, err)
		return nil, err
	}
	s := &S3Backend{
		bucket:    bucket,
...
func (s *S3Backend) detectBucketLocationByHEAD() (err error, isAws bool) {
	...
	s3Log.Debugf("detectBucketLocationByHEAD(1): HEAD %v = %v %v", u.String(), resp.StatusCode, region)
	if region == nil {
		for k, v := range resp.Header {
			s3Log.Debugf("detectBucketLocationByHEAD(2): %v = %v", k, v)
		}
	}
...
func (s *S3Backend) Init(key string) error {
	...
	if err != nil {
		if !isAws {
			...
			if err == unix.EACCES || err == fuse.EINVAL || err == unix.EAGAIN {
...
func (s *S3Backend) mpuCopyPart(from string, to string, mpuId string, bytes string, part int64,
	...
	s3Log.Debug("mpuCopyPart: ", params)
...
func sizeToParts(size int64) (int, int64) {
	...
	if size > MAX_S3_MPU_SIZE {
		panic(fmt.Sprintf("object putSize: %v exceeds maximum S3 MPU putSize: %v", size, MAX_S3_MPU_SIZE))
	}
...
func (s *S3Backend) mpuCopyParts(size int64, from string, to string, mpuId string,
	...
	MAX_CONCURRENCY := 100
	if MAX_CONCURRENCY > len(etags) {
		MAX_CONCURRENCY = len(etags)
	}
...
func (s *S3Backend) copyObjectMultipart(size int64, from string, to string, mpuId string,
	...
	if err != nil {
		...
		s3Log.Debug("copyObjectMultipart: ", params)
...
func (s *S3Backend) CopyBlob(param *CopyBlobInput) (*CopyBlobOutput, error) {
	...
	// make a shallow copy of the nodeClient so we can change the
...
func (s *S3Backend) MultipartBlobAdd(param *MultipartBlobAddInput) (*MultipartBlobAddOutput, error) {
	//en := &param.CommitPersistRecord.Parts[param.PartNumber-1]
	en := &param.Commit.Parts[0]
	...
	s3Log.Debug("MultipartBlobAdd: ", params)
...
func (s *S3Backend) MultipartBlobCommit(param *MultipartBlobCommitInput) (*MultipartBlobCommitOutput, error) {
	...
	s3Log.Debug("MultipartBlobCommit(1): ", mpu)
	...
	s3Log.Debug("MultipartBlobCommit(2): ", resp)
...
func (s *S3Backend) MultipartExpire(param *MultipartExpireInput) (*MultipartExpireOutput, error) {
	...
	for _, upload := range mpu.Uploads {
		...
		if !expireTime.After(now) {
			...
			if mapAwsError(err) == unix.EACCES {
				break
			}
		} else {
...
func (s *S3Backend) MakeBucket(param *MakeBucketInput) (*MakeBucketOutput, error) {
	...
	if s.config.BucketOwner != "" {
		...
		for i := 0; i < 10; i++ {
			...
			switch err {
			case nil:
			case unix.ENXIO, unix.EINTR:
				s3Log.Infof("ticketNumber for bucket")
